...
- 知识库可检索资料数为 16,118 条。
- 语义检索使用 7 个 Chroma 集合，共 25,155 条向量记录；海派文化资料已纳入独立集合。
- 查询模型使用开发机离线导出的 ONNX INT8 文件，服务器不执行在线量化；TF-IDF 保留为自动降级方案。
...
向量索引、嵌入模型和 ONNX 文件均不纳入 Git，通过受控的部署流程单独同步。2GB 服务器不得在启动阶段执行 PyTorch 动态量化；更新模型时应在开发机导出 ONNX 成品并先运行 `scripts/verify_vector_index.py`。
